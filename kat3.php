<!DOCTYPE html>
<html lang="tr">
<head>
    <meta charset="utf-8">
    <title>Collesium Mall - Merter / Istanbul / Turkey</title>
    <link rel="icon" href="images/icon.png" type="image/gif" sizes="16x16">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="">
    <meta name="keywords" content="">
    <meta name="author" content="">

    <!-- CSS Files
    ================================================== -->
    <link rel="stylesheet" href="css/bootstrap.min.css" type="text/css" id="bootstrap">
    <link rel="stylesheet" href="css/plugins.css" type="text/css">
    <link rel="stylesheet" href="css/style.css" type="text/css">
    <link rel="stylesheet" href="css/color.css" type="text/css">

    <!-- color scheme -->
    <link rel="stylesheet" href="css/colors/brown.css" type="text/css" id="colors">
</head>

<body class="has-menu-bar">

<?php include "header.php" ?>

<div id="background" data-bgimage="url(images/background/bg6.jpg) fixed"></div>
<div class='slider-overlay' style="position: fixed !important;top: 0; left: 0;"></div>
<div id="content-absolute">

    <!-- subheader -->
    <section id="subheader" class="no-bg">
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center">
                    <h4>Collesium</h4>
                    <h1>Kat 3</h1>
                </div>
            </div>
        </div>
    </section>

    <section id="section-main" class="no-bg no-top" aria-label="section-menu">
        <div class="container">
            <div class="row g-4">
                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image text-center">
                            <a href="silvio_massimo.php">
                                <img src="images/COLL_KATLAR/silvio_massimo_logo.webp" class="img-fluid" alt="" width="100%" style="object-fit: cover">
                                <img src="images/magazalar/0.1.jpg" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>SILVIO MASSIMO</h3>
                        </div>
                    </div>
                </div>


                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="intellectual.php">
                                <img src="images/COLL_KATLAR/intellectual_logo.jpg" class="img-fluid" alt="" style="width: 100%; object-fit: cover;">
                                <img src="images/magazalar/0.1.jpg" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>INTELLECTUAL</h3>
                        </div>
                    </div>
                </div>


                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="dragoon.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/magazalar/0.1.jpg" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>DRAGOON</h3>
                        </div>
                    </div>
                </div>

                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="magaza_detay.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/logo/logo-silvio_massomo.png" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>CYK (ÇAYKARA TEKSTİK)</h3>
                        </div>
                    </div>
                </div>


                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="magaza_detay.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/logo/logo-silvio_massomo.png" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>ENİSSE</h3>
                        </div>
                    </div>
                </div>

                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="magaza_detay.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/logo/logo-silvio_massomo.png" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>BEST STATION PLUS</h3>
                        </div>
                    </div>
                </div>

                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="magaza_detay.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/logo/logo-silvio_massomo.png" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>COLLESIUM KUYUMCULUK</h3>
                        </div>
                    </div>
                </div>


                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="magaza_detay.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/logo/logo-silvio_massomo.png" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>KURDAL KURDAL</h3>
                        </div>
                    </div>
                </div>


                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="magaza_detay.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/logo/logo-silvio_massomo.png" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>OBAX</h3>
                        </div>
                    </div>
                </div>

                <div class="col-lg-4">
                    <div class="de-room">
                        <div class="d-image">
                            <a href="magaza_detay.php">
                                <img src="images/logo/logo-dragoon.png" class="img-fluid" alt="">
                                <img src="images/logo/logo-silvio_massomo.png" class="d-img-hover img-fluid" alt="">
                            </a>
                        </div>
                        <div class="d-text">
                            <h3>TOMY LION (EX TABELA)</h3>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </section>
    <!-- subheader close -->
    <?php include "footer.php"?>
</div>

<!-- Javascript Files
================================================== -->
<script src="js/plugins.js"></script>
<script src="js/designesia.js"></script>
</body>
</html>
